"""Sentry error-tracking setup for the MCP server.

Init is GATED on the ``SENTRY_DSN`` env var (loaded from .env by __main__ via
load_dotenv, or from the systemd EnvironmentFile in prod). When unset this is a
complete no-op — nothing is sent. The Starlette integration auto-attaches
(FastMCP serves a Starlette ASGI app), so unhandled request errors are captured
without any manual middleware.
"""
