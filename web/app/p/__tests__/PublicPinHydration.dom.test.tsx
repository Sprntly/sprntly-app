 ...
 *  (400, 100) of a 1000×500 stage → the dropped pin's static coords are a
 *  deterministic 40%/20%. */
...
describe("PublicPrototypeChrome — server-pin hydration", () => {
  ...
  it("test_local_pin_keys_offset_beyond_hydrated_range: a locally-dropped pin renders once, numbered past the hydrated range", async () => {
    ...
    // Post-remap join sanity: the local marker is VISIBLE (an unmapped
    // occlusion set would have filtered it) and renders at ITS OWN dropped
    // coords (40%/20% from the stubbed stage click) — a mis-joined position
    // record would pull a hydrated pin's coords instead.
