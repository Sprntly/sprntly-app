...
// ─────────────────────── GithubInstallsSlot ───────────────────────
//
// Fetches all Sprntly App installations for the current company and
// renders a GithubRepoPicker per install. Most users will have one;
// the helper handles 0-or-many cleanly.

function GithubInstallsSlot({ onChanged }: { onChanged?: () => void }) {
  ...
  if (loading) return <p className="conn-config-meta-v">Loading installations…</p>
  if (error) return <p className="conn-config-error">Could not load installations: {error}</p>
