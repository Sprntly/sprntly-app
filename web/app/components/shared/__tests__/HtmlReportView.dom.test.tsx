...
describe("HtmlReportView", () => {
  ...
  it("trims the page gutter inside the panel, without touching the document's own styles", () => {
    ...
    expect(srcdoc).toContain(".page{padding-left:20px!important;padding-right:20px!important;")
    // Appended INSIDE head, after the document's own <style>, so it wins on
    // order rather than by editing the skill's template.
    expect(srcdoc.indexOf(".page{padding-left")).toBeGreaterThan(srcdoc.indexOf(".page{max-width:900px}"))
    expect(srcdoc).toContain("<h1>Voice of Customer</h1>")
  })

  it("strips the sheet's own frame so the panel draws only one border", () => {
    // The competitive-intelligence template paints `.page` as a bordered,
    // shadowed sheet on a tinted desk. Inside the panel — which already frames
    // this iframe in a bordered card — that reads as a second grey border
    // wrapped around the first, with a band of desk between them.
    const { container } = render(<HtmlReportView html={REPORT} fitPanel />)
    const srcdoc = container.querySelector("iframe")!.getAttribute("srcdoc")!
    expect(srcdoc).toContain("border:0!important")
    expect(srcdoc).toContain("border-radius:0!important")
    expect(srcdoc).toContain("box-shadow:none!important")
    // The desk is painted out to the page's own colour, so the sheet reaches the
    // panel's frame instead of floating inside a band of another shade.
    expect(srcdoc).toContain("body:has(.page){background:var(--page,#fff)!important;padding:0!important;}")
  })

  it("leaves the sheet's VERTICAL padding alone", () => {
    // Templates park absolutely-positioned page furniture in the sheet's top
    // margin — the competitive-intelligence report hangs its "Generated by
    // Sprntly" brandmark at `top:16px` inside `.page`. Zeroing the padding (as
    // the first cut of this override did) dropped that straight onto the <h1>.
    const { container } = render(<HtmlReportView html={REPORT} fitPanel />)
    const srcdoc = container.querySelector("iframe")!.getAttribute("srcdoc")!
    // Pinned whole, so the rule can't quietly grow a vertical padding again —
    // asserting the absence of one is only as good as the fixture it reads.
