// View tests for onboarding step 03 — "Share your business context."
...
describe("ContextUploadView — drafted-from-website hint", () => {
  it("pre-fills the paste box and shows the drafted hint when context came from analysis", () => {
    const html = render({
      pastedText: "Acme reconciles payments for SMBs.",
      contextDrafted: true,
    })
    // value is rendered into the textarea
    expect(html).toContain("Acme reconciles payments for SMBs.")
    // subtle "drafted from your website" hint
    expect(html).toContain("Drafted from your website")
  })

  it("renders an EMPTY paste box and NO drafted hint when analysis is absent", () => {
    const html = render({ pastedText: "", contextDrafted: false })
    expect(html).not.toContain("Drafted from your website")
    // the paste textarea is present but empty
    expect(html).toContain("Paste context")
  })
})
