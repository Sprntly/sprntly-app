...
/**
 * Onboarding page 03 — "Share your business context."
 *
 ...
 * every agent reasons through. The "Paste context" box is PRE-FILLED with
 * the business-context blurb drafted from the website analysis (step 1's
 * background call); the user can edit, clear, or replace it. If analysis
 * hasn't returned, the box starts empty and never blocks. Uploads (and the
 * synthesized paste/links file) are ingested into the company corpus via the
 * same path the Sources screen uses (POST /v1/datasets/{slug}/files).
 */
...
export type ContextUploadViewProps = {
  ...
  dragging: boolean
  hasAnything: boolean
  /** True when the paste box was seeded from the website-analysis blurb. */
  contextDrafted: boolean
  ...
  return (
    <>
      ...
      <div className="field">
        ...
        {contextDrafted && (
          <p className="ob-ctx-drafted-hint" role="status">
            Drafted from your website — edit or replace anything that&apos;s off.
          </p>
        )}
        <textarea
...
export function Onboarding3() {
  ...
  const [contextDrafted, setContextDrafted] = useState(false)
  // The user owns the box after their first keystroke; we then stop
  // overwriting it even if a late analysis result arrives.
  const userEditedRef = useRef(false)

  // Pre-fill the paste box from the website-analysis blurb when it arrives.
  // Best-effort: if analysis is pending or absent, the box stays empty (and
  // never blocks). We only seed while the user hasn't touched the box and it
  // is still empty, so we never clobber typed input.
  useEffect(() => {
    const blurb = websiteAnalysis?.business_context?.trim()
    if (!blurb) return
    if (userEditedRef.current) return
    setPastedText((prev) => {
      if (prev.trim().length > 0) return prev
      setContextDrafted(true)
      return blurb
    })
  }, [websiteAnalysis])
  ...
  function changePastedText(v: string) {
    userEditedRef.current = true
    setContextDrafted(false)
    setPastedText(v)
  }

  // Upload everything to the company corpus, then advance to step 4.
  ...
  return (
    ...
    >
      <ContextUploadView
        ...
        contextDrafted={contextDrafted}
        onPickFiles={addFiles}
        onRemoveStaged={removeStaged}
        onChangePastedText={changePastedText}
