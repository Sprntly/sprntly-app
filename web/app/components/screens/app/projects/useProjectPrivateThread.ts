...
export type UseProjectPrivateThreadOpts = {
   ...
   *  chat-edit target (parity with main chat's own open-tab `prd_id`). `null`
...
export function useProjectPrivateThread(
  ...
  // The PRD open in the artifact drawer beside this chat — the explicit edit
  // target both chat surfaces bind (parity with main chat's own open-tab
  // `prd_id`). `null` when no PRD is open.
  const openPrdId = opts?.openPrdId ?? null
  ...
  const send = useCallback(
    ...
    (input: string | SendCommand) => {
      ...
      // `persistTurnPair` call here, or the pair would double-write. Applies
      // DIRECTLY through the shared editor — no confirm step, no proposal
      // arm. `targetPrdId` lets a caller override the drawer's current id
      // (the executor below resolves one from the classify envelope);
      // absent, falls back to the drawer's own `openPrdId`.
      const runEditPrd = async (instruction: string, targetPrdId?: number | null) => {
        try {
          const res = await projectsApi.prdChatEdit(
            projectId, instruction, targetPrdId ?? openPrdId ?? undefined, clientMessageId,
          )
          if (!res.edited) {
            // Terminal no-edit reply (flag off / no PRD open / denied target
            // / no-op instruction).
      ...
      // `answer`. Sends the LIVE open-drawer `prd_id` so an edit-phrased
      // message with a PRD open actually survives classify (rather than
      // always downgrading to the "open a PRD" clarify). Sequenced through
      // `ensureConversationId()` so a deictic message resolves against this
      // caller's own thread.
      ensureConversationId()
        .then((conversationId) => projectsApi.resolveIntent(projectId, question, { conversationId, prdId: openPrdId }))
        .then((envelope: ChatIntentEnvelope) => {
          dispatchChatIntent(
            envelope,
            { hasEditTarget: true, editTargetPrdId: openPrdId ?? null, ticketsTarget: null },
            {
              ...
              ...useChatIntentExecutors({
                // `prdId` is the switch's own resolved target
                // (`ctx.editTargetPrdId`, echoed back) — pass it through
                // rather than discarding it, falling back to the drawer's
                // own id if it is ever absent.
                onEditPrd: (instruction, prdId) => void runEditPrd(instruction, prdId ?? openPrdId ?? undefined),
    ...
    [
      ...
      persistTurnPair, opts?.onArtifactsChanged, nextPrompts.retire, nextPrompts.onSettled, openPrdId,
    ],
  )
  ...
  const pickOption = useCallback(
    (sourceTurnId: string, option: { id: string; title: string; instruction?: string }) => {
      ...
      projectsApi
        ...
        .then((res) => {
          const settlePick = (answer: string) => {
            setSessionTurns((prev) =>
              prev.map((t) =>
                t.id === pickId
                  ? {
          ...
          // Direct-apply union: `edited: true` applied immediately (no
          // confirm step), `edited: false` is the terminal no-edit reply.
          settlePick(res.edited ? res.summary || "Updated the PRD." : res.answer)
        })
        .catch(() => {
          setSessionTurns((prev) =>
            prev.map((t) =>
  ...
  return {
    turns, send, stop, pickOption, submitClarify, skipClarify,
