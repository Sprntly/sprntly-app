...
describe("ProjectGroupChat — styled group nodes carry classes, not bare divs (AC2)", () => {
  it("test_group_no_stayed_out_pill_rendered — a human-only last turn with no reply renders NO stayed-out pill (removed: debug-y internal state, not user-facing); the remaining GroupChatExtras nodes stay styled", async () => {
    groupTurnsMock.mockResolvedValue([
      turn({ id: 1, author_user_id: "u1", author_name: "Me", content: "solo aside" }),
    ])
    render(React.createElement(ProjectGroupChat, { projectId: 101 }))
    await screen.findByTestId("gc-msg-me")
    // The pill no longer renders, under any reply-status condition.
    expect(screen.queryByTestId("gc-stayed-out")).toBeNull()
    // The relocated CSS families that remain live in GroupChatExtras; the
    // deleted `.stayedOut*` family is gone from the stylesheet too.
    ...
    expect(extrasCss).not.toContain(".stayedOut")
  })
})
