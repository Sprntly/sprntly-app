/**
 * Convert the v2 PRD Markdown into a PrdState with v2 semantic-block
 * sections (`v2-prd-tldr`, `v2-prd-problem`, `v2-prd-hypothesis`,
 * `v2-prd-requirements`, `v2-prd-acceptance-criteria`, `v2-prd-metrics`,
 * `v2-prd-risks`, `v2-prd-milestones`, `v2-prd-dod`), plus the shared
...
function parseMetricPoint(raw: unknown): PrdV2MetricPoint | null {
...
function parseTldrBlock(body: string): PrdSection | null {
  ...
  return { type: "v2-prd-tldr", problem, fix, impact }
}
...
function parseProblemBlock(body: string): PrdSection | null {
  ...
  const impact: PrdV2ProblemImpactCell[] = impactRaw
    .map((cell) => {
      ...
      const out: PrdV2ProblemImpactCell = { label, value }
      if (typeof c.tone === "string") {
        out.tone = normalizeTone(c.tone)
      }
      return out
    })
    .filter((c): c is PrdV2ProblemImpactCell => c !== null)
  if (!userStory && impact.length === 0) return null
  return { type: "v2-prd-problem", userStory, impact }
}
...
function parseRequirementsBlock(body: string): PrdSection | null {
  ...
  const rows: PrdV2RequirementRow[] = parsed
    .map((raw) => {
      if (!raw || typeof raw !== "object") return null
      const r = raw as Record<string, unknown>
    ...
    .filter((r): r is PrdV2RequirementRow => r !== null)
  if (rows.length === 0) return null
  return { type: "v2-prd-requirements", rows }
}

function parseAcceptanceCriteriaBlock(body: string): PrdSection | null {
  const parsed = tryParseJson(body)
  if (!Array.isArray(parsed)) return null
  const rows: PrdV2AcceptanceCriterionRow[] = parsed
    .map((raw) => {
      if (!raw || typeof raw !== "object") return null
      const r = raw as Record<string, unknown>
      const id = toStr(r.id)
    ...
    .filter((r): r is PrdV2AcceptanceCriterionRow => r !== null)
  if (rows.length === 0) return null
  return { type: "v2-prd-acceptance-criteria", rows }
}
...
function parseMetricsBlock(body: string): PrdSection | null {
  ...
  const secondary: PrdV2MetricPoint[] = secondaryRaw
    .map(parseMetricPoint)
    .filter((m): m is PrdV2MetricPoint => m !== null)
  const guardrailsRaw = Array.isArray(r.guardrails)
    ? (r.guardrails as unknown[])
    : []
  const guardrails: PrdV2Guardrail[] = guardrailsRaw
    .map((raw) => {
      if (!raw || typeof raw !== "object") return null
    ...
    .filter((g): g is PrdV2Guardrail => g !== null)
  if (!primary.name && secondary.length === 0 && guardrails.length === 0) {
    return null
  }
  return { type: "v2-prd-metrics", primary, secondary, guardrails }
}

function parseRisksBlock(body: string): PrdSection | null {
  const parsed = tryParseJson(body)
  if (!Array.isArray(parsed)) return null
  const rows: PrdV2RiskRow[] = parsed
    .map((raw) => {
      if (!raw || typeof raw !== "object") return null
      const r = raw as Record<string, unknown>
    ...
    .filter((r): r is PrdV2RiskRow => r !== null)
  if (rows.length === 0) return null
  return { type: "v2-prd-risks", rows }
}

function parseMilestonesBlock(body: string): PrdSection | null {
  const parsed = tryParseJson(body)
  if (!Array.isArray(parsed)) return null
  const phases: PrdV2MilestonePhase[] = parsed
    .map((raw) => {
      if (!raw || typeof raw !== "object") return null
    ...
    .filter((p): p is PrdV2MilestonePhase => p !== null)
  if (phases.length === 0) return null
  return { type: "v2-prd-milestones", phases }
}
...
function parseSemanticBlock(name: string, _attrs: string, body: string): PrdSection[] {
  switch (name) {
    case "context-chip":
      // Shared with evidence v2 — same renderer handles both formats.
...
export function markdownToPrdV2State(markdown: string): PrdState {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    // Fenced ```chart``` block (PRD v2 templates can emit a chart in
