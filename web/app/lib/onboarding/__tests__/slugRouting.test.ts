...
describe("clampStep — out-of-range persisted indices", () => {
  ...
  it("clamps stale old-flow steps (6, 7) down to the last valid step", () => {
    // Existing users mid the old 7-step flow must NOT crash — they land on the
    // last valid new step.
...
describe("slugForStep — resume index → slug (clamped)", () => {
  it("maps each in-range index to its ordered slug", () => {
    ...
    expect(slugForStep(4)).toBe("coworkers")
...
describe("isOnboardingStepSlug", () => {
  it("accepts the 5 numbered slugs and rejects analyzing / unknown", () => {
