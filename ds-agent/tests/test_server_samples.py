...
def test_samples_listed(client):
    ...
    r = client.get("/api/agents/ds/samples", headers=headers)
    assert r.status_code == 200
...
def test_load_sample_sets_dataset(client):
    headers = _login(client)
    r = client.post("/api/agents/ds/load-sample", json={"sample_id": "saas_retention"}, headers=headers)
    assert r.status_code == 200
    state = client.get("/api/agents/ds/state", headers=headers).json()
...
def test_unknown_sample_404s(client):
    headers = _login(client)
    r = client.post("/api/agents/ds/load-sample", json={"sample_id": "bogus"}, headers=headers)
...
def test_reset_clears_dataset(client):
    ...
    client.post("/api/agents/ds/reset", headers=headers)
    state = client.get("/api/agents/ds/state", headers=headers).json()
