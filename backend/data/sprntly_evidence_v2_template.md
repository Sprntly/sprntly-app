...
Render as a structured card so an engineer can pick this up and build the test from this block alone.

:::experiment
{
  "change": "[The specific change — e.g. 'Move the deductible disclosure from step 4 to step 1 and frame it as part of the upfront price.']",
  "primary_metric": {
    "name": "[e.g. Funnel completion rate]",
    "current": "[43%]",
    "target": "[58%]",
    "mechanism": "[Why this change moves this metric, derived from the evidence above. One sentence.]"
  },
  "secondary_effects": [
    "[Optional second-order effect — e.g. 'Expect a 1–2pp dip in step-1 entry rate from the heavier disclosure.']"
  ],
  "sample_size": "[e.g. 12k sessions per arm for 95% power on a 4pp lift]",
  "duration": "[e.g. 3 weeks]",
  "risks": [
    "[Risk 1 — what could go wrong or invalidate the result]"
  ]
}
:::
...
| Experiment card is the deliverable | If the experiment block isn't specific enough to A/B test from, the analysis is not finished. |
