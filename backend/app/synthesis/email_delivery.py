...
def render_report_ready_email(report_label: str) -> tuple[str, str, str]:
    """Render (subject, html_body, text_body) announcing a scheduled monthly
    report — the ready-ping's one-line-plus-CTA layout, pointed at the
    artifacts library where the report was saved. Pure + deterministic."""
    from app.synthesis.delivery import artifacts_deep_link

    url = artifacts_deep_link()
    cta = "Open your reports"
    line = f"Your monthly {report_label} is ready."
    subject = f"Your monthly {report_label} is ready"
    text_body = f"{line}\n\n{cta}: {url}"
    ...
    open_btn = (
        ...
        f'background:{_GREEN};color:#ffffff;text-decoration:none;'
        f'font-family:{_SANS};font-weight:600;font-size:14px;'
        f'padding:11px 20px;border-radius:9px;">{html.escape(cta)}</a>'
    )
    html_body = (
        '<!DOCTYPE html><html><head><meta charset="utf-8">'
        '<meta name="viewport" content="width=device-width,initial-scale=1.0">'
        ...
        '<div style="max-width:600px;margin:0 auto;">'
        f'<div style="font-family:{_SANS};font-size:16px;line-height:1.6;'
        f'color:{_INK_2};margin-bottom:20px;">{html.escape(line)}</div>'
        f'<div>{open_btn}</div>'
        f'<div style="font-family:{_SANS};font-size:12px;color:{_INK_SOFT};'
        f'margin-top:26px;line-height:1.5;">You are receiving this because brief '
        'notifications are enabled for your company in Sprntly.</div>'
...
def deliver_report_ready_to_email(company_id: str, *, report_label: str) -> dict:
    """Best-effort email announcement of a scheduled monthly report. Same
    gates and recipient resolution as the brief pings (email_enabled toggle,
    ...
    try:
        ...
        subject, html_body, text_body = render_report_ready_email(report_label)
        ...
        for addr in recipients:
            try:
                ...
                logger.exception("report ping email delivery failed for %s", addr)
        ...
        logger.exception("report ping email delivery failed for %s", company_id)
