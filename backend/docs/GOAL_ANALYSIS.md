...
before writing code in this area. Section 1 of the spec (the nine invariants) is
...
Run 2026-08-17 against **Chaostrack Inc.** (`e09ff4d4-515e-4fa4-8be7-67d2a85a6370`),
read-only, no writes, no LLM calls. Scripts: `backend/scripts/crucible_spike.py`,
`crucible_verify.py`, `crucible_probe.py`. These are throwaway research tools, not
product code.
...
That shape matters: Chaostrack is `enterprise_low_n` / `b2b_sales_led`. ARR sizing is
...
The spike surfaced a finding the shipped ranking places at **#631**:

> **ServiceNow integration is described to customers as already shipped and to
> prospects as two weeks away, and has been in that state for about three months.**
> Cencora requested it 2026-04-17. The Mosaic Company recorded it as a hard
> requirement 2026-05-06 — *"currently planned but not yet available."* Through
> 2026-07-15→07-24 it is repeatedly promised to Grand Casino as *"expected in 1–2
> weeks"*, and Arista Networks is expecting it. In the same window, capability
> statements assert as fact that *"ChaosTrack integrates with Slack and ServiceNow"*
> (2026-07-06, 07-23, 07-26, 07-27).

Two claim sets that are individually unremarkable and jointly a problem. Neither
account's own thread contains it. This is the two-documents shape, on real data.

**The first framing of this finding was wrong, and verification killed it.** The spike
initially proposed "the 1–2 weeks promise recurs over months." Pulling all 56
ServiceNow-mentioning signals in date order showed every ETA promise clustered in a
single week (07-15→07-24) and mostly echoing one 07-15 demo. Had that shipped it
would have been confident, well-sourced, and false — spec F1 exactly. **Adversarial
verification is a pipeline stage, not an optional extra.**

Honest precision: the detector produced **4 candidates, 1 real** (`POC / Trial` is a
category not a finding; `sales pipeline / deal timing` is generic; `pricing model` is
mildly interesting). 25% is a starting point, not a good number.
...
affected. The measured effect on Chaostrack: the top-ranked theme is
`procurement / contract vehicles` — **2 claims, 0 named accounts**. Meanwhile
`sales deal progression` (19 accounts) ranks #199, `tabletop exercises` (16 accounts)
#161, and the ServiceNow cluster #631.
...
- **Theme dedup is load-bearing.** `sales pipeline` / `Sales / Deal Progression` /
  `sales deal progression` / `sales pipeline / deal progression` are one theme in
  four rows, splitting its accounts four ways. 2,141 themes → 2,067 on a crude label
  merge. The spec's "dedupe by mechanism, not by wording" is required, not polish.
- **Signal duplication inflates every count.** Grand Casino's integration request
  appears ~8 times across 07-21→07-26 as near-identical rows.
...
## 3. The nine invariants, and where each lands
...
All nine invariant tests green in CI. Staging Chrome-verified. **No prod cutover.**
...
- **Goal currency assumes linear value per unit.** Every adoption-driven number is
  overstated by an unknown amount (critique weakness 3). On Chaostrack there is no
  value-per-account data at all, so accounts-as-currency is itself an assumed
