"""Tests for the timezone-aware top-insights schedule (v0 checklist 2.4).

The Top Insights brief must fire **Monday 06:00 in the company owner's timezone**. The
...
Coverage:
  ...
  - should_run_brief across ≥2 timezones (New York + Sydney) — the SAME
...
def test_due_at_monday_0600_in_each_timezone():
    ...
    assert should_run_brief(ny_fire, ny, None) is True
    ...
    assert should_run_brief(syd_fire, syd, None) is True
...
def test_same_utc_instant_due_in_one_tz_not_another():
    ...
    assert should_run_brief(instant, ny, None) is True
    assert should_run_brief(instant, syd, None) is False
...
def test_dst_shifts_the_utc_fire_instant_for_new_york():
    ...
    assert should_run_brief(winter_fire, ny, None) is True
    ...
    assert should_run_brief(summer_fire, ny, None) is True
...
def test_already_ran_this_week_is_not_due_again():
    ...
    assert should_run_brief(first_tick, ny, None) is True
    ...
    assert should_run_brief(second_tick, ny, first_tick) is False
...
def test_last_run_in_a_prior_week_does_not_suppress_this_week():
    ...
    assert should_run_brief(this_week, ny, last_week) is True
...
def test_naive_datetimes_are_treated_as_utc():
    ...
    ny = ZoneInfo("America/New_York")
    assert should_run_brief(datetime(2026, 6, 8, 10, 0), ny, None) is True
...
def test_generation_once_per_cycle_guard():
    ...
    assert should_generate_brief(first, ny, None) is True
    ...
    assert should_generate_brief(first, ny, last_week) is True
...
class TestShouldRunByFrequency:
    def test_daily_weekdays_is_due_on_tuesday_but_not_saturday(self):
        ...
        assert should_run_brief(tue, UTC_ZONE, None, **kw) is True
        # Saturday isn't a fire day at all, so the most recent fire is Friday's
        # — far outside the 1h due window.
        assert should_run_brief(sat, UTC_ZONE, None, **kw) is False
    ...
    def test_daily_weekdays_reruns_the_next_day(self):
        ...
        assert should_run_brief(mon, UTC_ZONE, mon, **kw) is False
        assert should_run_brief(tue, UTC_ZONE, mon, **kw) is True
    ...
    def test_biweekly_skips_the_off_week(self):
        ...
        assert should_run_brief(on, UTC_ZONE, None, **kw) is True
        assert should_run_brief(off, UTC_ZONE, on, **kw) is False
        assert should_run_brief(next_on, UTC_ZONE, on, **kw) is True
    ...
    def test_monthly_runs_once_per_month(self):
        ...
        assert should_run_brief(first_mon_aug, UTC_ZONE, None, **kw) is True
        assert should_run_brief(
    ...
    def test_absent_frequency_behaves_exactly_like_weekly(self):
        ...
        mon = datetime(2026, 7, 20, 6, 0, tzinfo=UTC)
        tue = datetime(2026, 7, 21, 6, 0, tzinfo=UTC)
        assert should_run_brief(mon, UTC_ZONE, None, **kw) is True
        assert should_run_brief(tue, UTC_ZONE, None, **kw) is False
    ...
    def test_generation_lead_still_applies_to_a_non_weekly_cadence(self):
        ...
        assert should_generate_brief(gen, UTC_ZONE, None, **kw) is True
        assert should_generate_brief(
