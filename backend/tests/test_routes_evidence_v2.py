"""Tests for app.routes.evidence_v2 — POST /v1/evidence/v2/generate
and GET /v1/evidence/v2/{id}.

Mirrors test_routes_evidence.py, with one extra case: v1 and v2 rows are
stored in the same table but must not dedupe against each other.
"""
...
# ---- GET /v1/evidence/v2/{id} ----------------------------------------------

def test_get_nonexistent_v2_evidence_returns_404(app_client, isolated_settings):
    resp = app_client.get("/v1/evidence/v2/9999")
    assert resp.status_code == 404


def test_get_v2_evidence_without_auth_returns_401(unauth_client, isolated_settings):
    resp = unauth_client.get("/v1/evidence/v2/1")
...
def test_get_v2_endpoint_rejects_v1_row(app_client, isolated_settings):
    ...
    v1_id = db_mod.start_evidence(
        brief_id=brief_id, insight_index=0, title="t", template_version=1
    )  # default variant='v1'
    db_mod.complete_evidence(v1_id, title="t", md="# v1 body")

    resp = app_client.get(f"/v1/evidence/v2/{v1_id}")
    assert resp.status_code == 409


def test_get_v2_evidence_returns_row(app_client, isolated_settings):
    db_mod = isolated_settings["db"]
    brief_id = _save_brief_with_insights(db_mod)
    v2_id = db_mod.start_evidence(
        brief_id=brief_id,
        insight_index=0,
        title="t",
        template_version=1,
    ...
    db_mod.complete_evidence(v2_id, title="t", md="# v2 body")

    resp = app_client.get(f"/v1/evidence/v2/{v2_id}")
...
# ---- POST /v1/evidence/v2/generate -----------------------------------------

def test_generate_v2_without_auth_returns_401(unauth_client, isolated_settings):
    resp = unauth_client.post(
        "/v1/evidence/v2/generate", json={"brief_id": 1, "insight_index": 0}
    )
    assert resp.status_code == 401
...
def test_generate_v2_missing_brief_returns_404(app_client, isolated_settings):
    resp = app_client.post(
        "/v1/evidence/v2/generate", json={"brief_id": 9999, "insight_index": 0}
    )
    assert resp.status_code == 404
...
def test_generate_v2_returns_existing_doc_when_not_forced(
    ...
    db_mod.complete_evidence(existing_id, title="t", md="# Already here (v2)")
...
def test_generate_v2_does_not_dedupe_against_v1_row(
    ...
    """A ready v1 doc for the same (brief, insight) must NOT be returned by
