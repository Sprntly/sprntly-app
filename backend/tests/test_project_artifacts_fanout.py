...
def _seed_prd(*, brief_id: int, title: str, insight_index: int = 0, status: str = "ready") -> int:
    from app.db.client import require_client

    resp = require_client().table("prds").insert(
        {"brief_id": brief_id, "insight_index": insight_index, "title": title, "status": status}
    ).execute()
    return resp.data[0]["id"]
