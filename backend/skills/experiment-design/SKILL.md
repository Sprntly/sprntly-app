...
4. **Sample size & duration** — estimate from baseline, MDE, power (0.8), alpha (0.05). Compute it yourself and SHOW the arithmetic, so the reader can check it; state the baseline and MDE you assumed.
...
## Output spec
Hypothesis · primary + guardrail metrics · MDE · sample size + duration estimate (with assumptions) · assignment design · pre-registered analysis & stopping rule · risks. Show the sizing arithmetic inline.
