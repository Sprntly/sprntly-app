...
- **Inputs from Sprntly:** problem + ranked evidence from the Weekly Brief; the **constitution from `business-context`/knowledge graph** (so the PM agent inherits engineering constraints, never re-asks); DS/Synthesis agents' analysis; the connected codebase/design system/prototype as grounding for Part B.
