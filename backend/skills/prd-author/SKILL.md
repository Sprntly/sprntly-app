...
8. **Done-when.** A testable exit condition for the cycle.

## Output spec
A single human-readable document with sections: Context · TL;DR · Problem & evidence · Hypothesis · Goals & metrics (primary vs guardrails, with formulas + baselines) · Scope & non-goals · Users & scenarios · Requirements (with `[edge case]` / `[failure]` tags where they matter) · Acceptance criteria (Given/When/Then) · Risks + the one riskiest assumption (3-line pre-mortem) · Open questions (owners) · Milestones · Done-when. Render it using `templates/prd-template.md`. **Optional Word export** via the `docx` skill.
