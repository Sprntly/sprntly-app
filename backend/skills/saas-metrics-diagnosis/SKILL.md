...
1. **Compute** whichever metrics the inputs support (LTV, CAC, LTV/CAC, payback, churn, magic number, quick ratio, etc.). Show each formula with the numbers substituted, so every figure can be checked.
...
## Output spec
Computed metrics table · benchmark comparison · the identified bottleneck + why · the quantified prize of fixing it · the prescribed lever + explicit "don't optimize X yet." Show each formula with its numbers substituted.
