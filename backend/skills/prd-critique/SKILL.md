...
1. **Structural lint.** Walk this checklist explicitly, item by item: missing problem statement, no baseline on the primary metric, no guardrail, no non-goals, no "done when", solution language in the problem section. Report each as present or absent — these are structural facts, not opinions, so do not soften them.
