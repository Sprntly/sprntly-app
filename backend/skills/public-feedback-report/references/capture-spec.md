...
**Sweep the standing channel list every run** — Reddit, X, Apple App Store, Google Play, Trustpilot, G2, Capterra, TrustRadius, Gartner Peer Insights, YouTube, TikTok, Hacker News, Facebook Groups, Discord and forums, GitHub, Quora, Product Hunt, marketplace review pages, Q&A and help sites, news, analyst coverage, blogs and newsletters. Where the product ships inside an app, the app-store listings are **mandatory**, including the parent or companion app when the feature has no listing of its own — that is where dated, version-tagged complaints live. Record every channel that returned nothing, by name, so a gap reads as a gap rather than as quiet.
