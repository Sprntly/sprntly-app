...
4. **Do the math in the open.** Score every item with the chosen framework and SHOW the per-factor numbers, so the ranking can be checked rather than trusted. Two modes: *plain* ranks by the framework alone; *goal* additionally weighs each item's fit against the North Star (and any secondary metrics), and must print both the raw and the goal-adjusted score so the effect of that weighting is visible. State which mode you used.
...
Rank by the modeled North-Star impact, and carry a low/high range when the estimate is uncertain rather than a single false-precise number.
...
State the framework (rice / wsjf / voc / northstar) and the mode (plain / goal) you used, and show the per-factor scores behind the ranking.

## Sprntly integration (optional)
- **Inputs from Sprntly:** the **goal from `business-context`** (so prioritization is scored against the company's actual stated goal); backlog items + confidence scores from the Top Insights / outcome graph (Reach and Confidence can be data-derived, not guessed).
