...
Every run reads a state file, diffs against it, and rewrites it. Without stored state, "what changed" is a memory exercise, and memory is where fabrication enters.
...
| Stage | Question |
|---|---|
| **The arena** | Direct rivals, substitutes, adjacent and future entrants (Porter's Five Forces). *First section cut when the page budget is tight — fold its conclusion into the threat scan.* |
...
**A PDF. Always.** The deliverable handed to the caller is a PDF file — never a raw HTML file, never markdown, never chat-only prose. Build a single self-contained HTML document as the source, then render it to PDF and present the PDF.

Render with headless Chromium (Playwright) at A4 with backgrounds on, not with a text-only PDF library — the radar charts are inline SVG and the confidence chips are styled spans, and both are lost by a naive converter. Three print rules matter, because each has silently broken a run: A4 content width is ~700 CSS px, which trips any `max-width:820px` mobile breakpoint and collapses the two radars into one column, so re-assert the desktop grids inside `@media print`; radar `viewBox` must extend past the plot area or the longest axis labels clip at the edge; and `break-inside:avoid` on a block taller than the remaining page leaves most of a page blank, so cap chart height in print rather than letting a tall block jump. Rasterise two or three pages and look at them before delivering.

**Eight to ten pages. Ten is a hard cap.** Enforce it by measuring, not by estimating: render, count pages, adjust, re-render, and repeat until it lands in range. Aim for nine so a later run that adds a competitor does not blow the cap. Two checks before accepting a result — rasterise each page and confirm no page ends below roughly 60% fill (a near-empty page means an unbreakable block jumped and a page is being wasted), and confirm body type has not been shrunk past readability. Reclaim space structurally before shrinking type: let recommendation cards and tables break across pages rather than jumping whole, and tighten section spacing, which is worth more than it looks across a dozen sections.

When the budget still will not close, cut in this order, and never by dropping a benchmark, the radar, the threat scan or the recommendations:
1. **The arena (Five Forces)** comes out first. Its conclusion is one sentence and belongs at the end of the threat scan, which is already rating the same pressures with more precision.
2. **The launch log condenses before it disappears.** Keep the launches that changed a position — four to six for the heavy shippers — and carry the rest inside the pattern line, which still names the count, the cadence and the deprecations. Competitors with a handful of releases collapse from a table into a short dated paragraph, keeping date, classification and pattern.
3. **Prose tightens last.** Cut repetition, never a finding.

Order:

0. **Attribution line** — `Generated by Sprntly AI`, small, above the title. This is the one permitted exception to the rule that nothing about the report's production appears in the report: it is a fixed mark of origin, not a note about cadence, audience or method.
...
- [ ] Delivered as a PDF of eight to ten pages, verified by rendering and counting — with the attribution line present, both radars side by side, no clipped axis labels, no near-blank pages and body type still readable.
