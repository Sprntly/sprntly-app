...
**The deliverable is always a PDF of eight to ten pages**, rendered from a self-contained HTML source via headless Chromium so the radar charts and confidence chips survive. It carries a `Generated by Sprntly AI` line above the title.

The page budget is enforced by measurement, not estimation: render, count, adjust, re-render until it lands in range, then check that no page is left near-empty by a block that jumped. When the budget will not close, the sanctioned cut order is the arena first, then condensing the launch log to the launches that changed a position, then tightening prose. Benchmarks, the radar, the threat scan and the recommendations are never cut.
...
- `examples/02-sprntly.html` — full run: Sprntly against Productboard Spark, ChatPRD, Linear, Claude Code and Amplitude
